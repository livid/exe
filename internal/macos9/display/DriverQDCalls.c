...
GraphicsCoreGetModeTiming(VDTimingInfoRec *timingInfo)
{
	...
	timingInfo->csTimingFlags	=
		(1 << kModeValid) | (1 << kModeSafe);

    /* exe: the restricted EDID list is 640x480, 800x600, 1024x768.
       Only the 800x600 mode is the default; marking every mode as default
       makes classic Mac OS choose the first (640x480) at startup. */
    if (timingInfo->csTimingMode == 2)
